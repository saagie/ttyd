...
# Spark (local only)
RUN cd app \
    && wget http://apache.mirror.anlx.net/spark/spark-2.3.3/spark-2.3.3-bin-hadoop2.6.tgz \
    && tar -xvzf spark-2.3.3-bin-hadoop2.6.tgz \
    && cp spark-2.3.3-bin-hadoop2.6/conf/spark-env.sh.template spark-2.3.3-bin-hadoop2.6/conf/spark-env.sh \
    && echo 'export HADOOP_CONF_DIR=/etc/hadoop/conf/' >> spark-2.3.3-bin-hadoop2.6/conf/spark-env.sh \
    && rm spark-2.3.3-bin-hadoop2.6.tgz

ENV PATH "/app/spark-2.3.3-bin-hadoop2.6/bin:${PATH}"
